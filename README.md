...
- `apps/web/index.html`: landing page (upload + parse-and-tweak flow)
- `apps/web/viewer.html`: animated phone viewer + customization card + standalone HTML download
- `apps/web/parser.html`: optional advanced parser page
...
2. Pick one input:
   ...
   - Upload WhatsApp TXT.
3. Click **Parse and tweak** to open `apps/web/viewer.html` with parsed data loaded.
...
- `apps/web/` - frontend pages (landing, viewer, parser)
...
- `legacy/` - deprecated reference artifacts

## Legacy

`scripts/generate_report.py` and `report/report.html` are deprecated and preserved for reference.
